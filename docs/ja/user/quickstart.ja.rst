...
continuing.
もしまだ導入をしないのであれば、進める前にFalconをインストールしてください。
...
上記の例は、uWSGIやGunicornなどのWSGIサーバーを使用して実行できます。 例
...
GunicornとuWSGIがまだ動作しないWindows上では、Waitressを使用できます。
...
Curlを使用するのは少し面倒であるため、HTTPieをインストールしてから使用しましょう。
...
ここでは、ヘッダーとクエリパラメータの読み取り、エラーの処理、および要求と応答の本体の操作を示す、より多くの関連する例を示します。
ここでは、ヘッダーとクエリのパラメータを読み取る方法、より複雑な例を示します。
